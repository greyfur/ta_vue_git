<template>
  <div class="detailPay">
      ...
      <el-col :span="11" style="padding:0 16px;">
        ...
        <div class="btn" v-if="$route.query.tag === 'payClose'">
          ...
          <el-button
            ...
            @click="gangUp('核销')"
            plain
          >{{!hxState?'挂起':'悬停'}}</el-button>
          <el-button size="small" :disabled="hxState" plain @click="submite(1,'流程提交')">流程提交</el-button>
        </div>
        <!-- 财务支付/紧急付款/partial完结 -->
        <div
          class="btn"
          v-if="$route.query.tag === 'payment' || $route.query.tag === 'partialDone' || $route.query.tag === 'instancyPay'"
        >
          ...
          <el-button
            ...
            plain
            @click="tongbu"
            v-if="$route.query.tag === 'instancyPay'"
          >同步状态</el-button>
        ...
        <!-- 失败的step -->
        <!-- <el-steps finish-status="success" :active="this.row.approvalLevel">
              <el-step v-for="(item,index) in strArrCre" :key="item" :title="index===0?item.done:item.wait" process-status="wait"></el-step>
        </el-steps>-->
        <!-- <div class="btn approvalDoneBtn" v-if="$route.query.tag === 'payVerification'">
          <div class="approvalDone">
            <span class="approvalWrap" v-for="(item,index) in strArr" :key="item">
              <span class="word">{{item}}杭一达</span>
        ...
        <div class="searchNew" style="border-bottom:none;margin-top:16px;">
          <div class="titleSearch detailSearch" @click="searchFlag2 = !searchFlag2">
            ...
            <p
              v-if="$route.query.tag != 'payClose'&&$route.query.tag != 'payReview' && $route.query.tag != 'payReview' && $route.query.tag != 'payVerification'"
            >
              <el-button size="mini" @click="mailSend(1,'上传附件')">
          ...
          <el-table
            stripe
            :data="fileData"
            style="width:100%;min-height:350px;max-height:500px;"
            class="document"
          >
            ...
            <el-table-column
              label="操作"
              width="100"
              v-show="$route.query.tag=='payOperation' || $route.query.tag =='approvalDone' || $route.query.tag=='payReview'"
            >
              <template slot-scope="scope">
                <el-button
                  v-show="$route.query.tag=='payOperation' || $route.query.tag =='approvalDone' || $route.query.tag=='payReview'"
    ...
    <el-row
      v-if="$route.query.tag !== 'payOperation' && $route.query.tag !== 'payVerification'"
    ...
    >
      <el-col :span="24">
        ...
        >
          ...
          <p v-if="$route.query.tag === 'approvalDone'">
            <el-button size="mini" @click="getRMSg">
        ...
        <el-collapse-transition>
          <el-table v-show="searchFlag2" stripe :data="RMData" style="width:100%">
            ...
            <el-table-column label="操作" width="140">
              <template slot-scope="scope">
                <el-button
                  @click="onOpenSICS(scope.row,'rmId')"
                  v-if="$route.query.tag === 'payClose' || $route.query.tag === 'payment' || $route.query.tag === 'instancyPay' || $route.query.tag === 'partialDone'"
                  type="text"
                  size="small"
                >Reverse</el-button>
              </template>
    ...
    <el-row style="padding:0 16px;">
      <el-col :span="24">
        ...
        >
          <div>
            <i style="margin-right:8px;" class="el-icon-arrow-down"></i>结算清单
          </div>
          <p v-if="$route.query.tag === 'payOperation' || $route.query.tag === 'approvalDone'">
            <el-button size="mini" @click="getSGSg">
              <i style="margin-right:8px;" class="iconfont iconGroup77"></i>SICS回写
            </el-button>
          </p>
        </div>
        <el-table v-show="searchFlag3" stripe :data="SgData" style="width: 100%">
          ...
          <el-table-column label="操作" width="120">
            <template slot-scope="scope">
              <el-button
                v-show="!scope.row.rmId"
                @click="remitCreat(scope.row)"
                v-if="$route.query.tag === 'approvalDone' || $route.query.tag === 'payClose'"
                type="text"
                size="small"
              >创建支票</el-button>
            </template>
      ...
      >
        <el-form-item label="经纪公司">
          <el-select clearable filterable v-model="makeDocList.rmBrokerName" placeholder="请选择">
            <el-option
              v-for="(item,index) in brokerListS"
              :key="index"
              :label="item.codecode+' - '+item.codeName"
              :value="item.codeName"
            >
        ...
        <el-form-item label="分出公司">
          <el-col :span="10">
            <el-select
              clearable
              filterable
              v-model="makeDocListEctype.cedentModel[0]"
              placeholder="请选择"
            >
              <el-option
          ...
          <el-col :span="10">
            <el-select
              clearable
              filterable
              v-model="makeDocListEctype.cedentModel[1]"
              placeholder="请选择"
            >
              <el-option
                v-for="(item,index) in cedentList"
          ...
          <el-col :span="10">
            <el-select
              clearable
              filterable
              v-model="makeDocListEctype.cedentModel[2]"
              placeholder="请选择"
            >
              <el-option
                v-for="(item,index) in cedentList"
        ...
        <el-form-item label="受款公司">
          <el-select
            clearable
            filterable
            v-model="makeDocListEctype.shoukuanMode"
            placeholder="请选择"
          >
            <el-option
              v-for="(item,index) in bscBankList"
              :key="index"
              :label="item.compName"
              :value="index"
            ></el-option>
        ...
        <el-form-item label="原币币制/金额" required>
          <el-form-item>
            <el-select
              ...
              placeholder="请选择"
              multiple
              v-model="makeDocListEctype.yuanType"
              class="curAmount"
              @change="selectChange"
            >
              <el-option
        ...
        <el-form-item label="折币币制/金额" required>
          <el-col :span="10">
            <el-form-item>
              <el-select
                v-model="makeDocListEctype.zheType"
                placeholder="请选择"
                class="curAmount"
                @change="zheTypeChange"
              >
                <el-option
                  v-for="item in rmCurrencyList"
                  :key="item.alpha"
...
export default {
  ...
  created() {
    ...
    if (
      this.$route.query.tag === "payOperation" ||
      this.$route.query.tag === "payClose"
    ) {
  ...
  mounted() {
    ...
    this.$http
      ...
      .then(res => {
        ...
        this.strArrCre = strArrCre;
      });
    this.mustData.actOperator = this.$store.state.userName;
    let strArr = [];
    if (
      this.row.approvalLevel != null ||
      this.row.approvalLevel != "undefined"
    ) {
      for (let i = 0; i < this.row.approvalLevel + 1; i++) {
        strArr.push(`${i + 1}级审批完成`);
      }
    }
    strArr[strArr.length - 1] = `待${strArr[strArr.length - 1].substr(
  ...
  methods: {
    ...
    getSGSg() {
      this.$http
        .post("api/sics/basis/getWSAndSGfromSics", {
          actOperator: this.mustData.actOperator,
          processId: this.row.processId
        })
        .then(res => {
      ...
      switch (tag) {
        case 1: // 流程提交
          if (this.$route.query.tag === "payOperation") {
            ...
            this.$http
              .post("api/sics/basis/getWFMessage", {
                processId: this.row.processId,
                actOperator: this.$store.state.userName
              })
              .then(res => {
        ...
        case 5: // 审批通过 --------------
          ...
          this.$http.post("api/docOperation/addSignature", param).then(res => {
            if (res.data.code == 0) {
              this.$http
                .post("api/pay/activitiForPay/getNextStep", {
                  processId: this.row.processId,
        ...
        case 6: // 复核通过  -------付款复核通过还缺 是否有支票的校验----7.15日，改为无选择人
          this.$http
            .post("api/sics/basis/getPayReviewMessage", {
              processId: this.row.processId
            })
            .then(res => {
        ...
        if (valid) {
          this.$http
            .post(
              ...
              Object.assign(this.formLabelAlign, this.mustData, { rmType: "P" })
            )
            .then(res => {
    ...
    remitCreat(row) {
      this.$http
        .post("api/sics/basis/creatPayRemitBySgNum", {
          sgNum: row.sgNum,
          actOperator: this.$store.state.userName,
          processId: this.row.processId
        })
    ...
    baseCompanyChange() {
      if (this.AllBankAccountList.length) {
        ...
        if (this.row.baseCompany === "China Re Group") {
          // 集团
          this.BankAccountList = Glist;
        } else if (this.row.baseCompany === "China Re P&C") {
          // 产再
    ...
    zheTypeChange() {
      ...
      if (this.makeDocListEctype.yuanType.length) {
          ...
          if (el) {
            ...
            if (curType != "CNY" && curType != "USD") {
              ...
              if (val) {
                let USD = 1 / val;
                // 再把美元转为另一个小汇率
                let val2 = this.filterCurrencyRateList(
                  "USD",
                  this.makeDocListEctype.zheType
                );
                if (val2) {
                  this.makeDocListEctype.yuanHuiLv[i] =
                    Number(val2 * USD) > 0
                      ? Number(val2 * USD).toFixed(5)
                      : null;
                  if (this.makeDocListEctype.yuanHuiLv[i] != null) {
                    allNum +=
                      this.makeDocListEctype.yuanNum[i] *
                      Number(1 / this.makeDocListEctype.yuanHuiLv[i]);
                  }
                }
              }
            } else {
              // 直接转换、、、
              let val3 = Number(
                this.filterCurrencyRateList(
                  curType,
                  this.makeDocListEctype.zheType
                )
              );
              this.makeDocListEctype.yuanHuiLv[i] =
                Number(val3) > 0 ? Number(val3).toFixed(5) : null;
              if (this.makeDocListEctype.yuanHuiLv[i] != null) {
                allNum +=
                  Number(this.makeDocListEctype.yuanNum[i]) *
                  Number(1 / this.makeDocListEctype.yuanHuiLv[i]);
              }
            }
          }
        ...
        this.makeDocListEctype.zheNum =
          allNum > 0 ? Number(allNum).toFixed(5) : null;
      }
    },
    makeDoc(tag, name) {
      // 生成审批文档
      if (tag == "a") {
        ...
        if (tag == 2) {
          ...
          if (this.makeDocListEctype.shoukuanMode != null) {
            this.makeDocList = Object.assign(
              {},
              this.bscBankList[this.makeDocListEctype.shoukuanMode],
              this.makeDocList
            );
          }
          if (this.makeDocListEctype.zheNum) {
            this.makeDocList.convertAmount = `${
              this.makeDocListEctype.zheType
            }${this.makeDocListEctype.zheNum}`;
          ...
          this.$confirm("确认审批", "提示", {
            ...
            this.$http
              .post("api/docOperation/addSignature", param)
              .then(res => {
                if (res.data.code == 0) {
                  // this.makeDocEcho();
                  this.$message({ type: "success", message: "成功!" });
  ...
  watch: {
    title: function(n, o) {
      ...
      if (n === "流程提交" && this.$route.query.tag === "payOperation") {
        ...
        n === "流程提交" &&
        this.$route.query.tag === "approvalDone" &&
        this.row.accountCloseFlag == "0"
      ) {
        this.putIn = "b";
      } else if (n === "流程提交") {
        this.putIn = "n";
    ...
    "makeDocListEctype.yuanHuiLv": {
      handler: function(n, o) {
        ...
        if (this.makeDocListEctype.yuanNum.length) {
          ...
          this.makeDocListEctype.zheNum =
            all > 0 ? Number(all).toFixed(5) : null;
        }
      }
    }
  }
