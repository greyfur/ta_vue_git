<template>
  <div class="remit">
    ...
    <el-table :data="tableData" border style="width: 100%">
      ...
      <el-table-column prop="partnerCode" width="110" label="结付公司代码"></el-table-column>
      ...
      <el-table-column label="Base Company" width="120">
      ...
      <el-table-column prop="modifiedBy" label="操作员"></el-table-column>
      ...
      <el-table-column prop="rmChargesCurrency" label="手续费币制"></el-table-column>
      <el-table-column prop="rmChargesAmount" label="手续费金额"></el-table-column>
      ...
      <el-table-column fixed="right" label="操作" width="170">
        <template slot-scope="scope">
          ...
          <el-button v-show="urlName === 'taskCreation'" @click.stop="handleClick(7,scope.row)" type="text" size="small">删除</el-button>
          <el-button @click.stop="handleClick(11,scope.row)" type="text" size="small">踪迹</el-button>
          <el-button v-show="urlName === 'taskCreation'" @click.stop="handleClick(8,scope.row)" type="text" size="small">上传附件</el-button>
    ...
    <el-dialog :title="title" :visible.sync="dialogFormVisible" :close-on-click-modal="modal">
      <el-form :label-position="labelPosition" label-width="140px" :model="formLabelAlign" :rules="rules" ref="formLabelAlign">
        ...
        <el-form-item label="汇款人名称" v-show="title!=='查询'">
          <el-input v-model="formLabelAlign.rmSettleCompanyName"></el-input>
        </el-form-item>
        <el-form-item label="汇款到账日期" v-show="title!=='查询'"> 
...
export default {
  ...
  methods: {
    init(tag){
      // 进首页查询
      this.$http.post('api/integeratedQuery/Remitlist',Object.assign({},this.mustData,{actName:this.$store.state.userName,actOperator:this.$store.state.userName})).then(res =>{
       console.log(res,',,,');
       if(res.status === 200 ) {
    ...
    handleClick(tag,row){
      ...
      switch(tag){
        ...
        case 6: //编辑
          ...
          // 结付公司代码回显
          if(this.chooseRow.partnerCode){
            this.cedentList.forEach((el,i)=>{
              if(el.codecode == this.chooseRow.partnerCode){
                this.cedentModel = i;
              }
            })
           }
    ...
    confirm(formName){
      ...
      switch(this.tag){
        ...
        // if(!this.formLabelAlign.processStatus){ this.formLabelAlign.processStatus = this.processStatusCom; }
          ...
          this.$http.post('api/integeratedQuery/Remitlist',params).then(res =>{
            if(res.status === 200){
